...
export function ShareButton({ certificateId, name }: { certificateId: string; name: string }) {
  ...
  const text = `${name} úspěšně dokončil/a 5denní vodní půst!`;

  const handleShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: "Certifikát - The-Pulse.cz", text, url });
      } catch {
  ...
  return (
    ...
    >
      ...
      {copied ? "Odkaz zkopírován!" : "Sdílet certifikát"}
    </button>
  );
}
