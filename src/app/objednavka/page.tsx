  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/create-checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, plan, name, gender, goal }),
      });

      const data = await res.json();

      if (!res.ok) {
        setError(data.error || "Nepodařilo se vytvořit platební session.");
        setLoading(false);
        return;
      }
          ...
          {step === 3 && (
            ...
            >
              <div className="bg-white rounded-2xl shadow-xl border border-teal-100 p-8">
                ...
                <p className="text-navy-500 text-sm text-center mb-8">
                  Vyberte plán a pokračujte k platbě
                </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {isAuthenticated ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <button
                        type="button"
                        onClick={() => signIn("google", { callbackUrl: `/objednavka?plan=${plan}` })}
                  ...
                  >
                    {loading ? "Přesměrování na platbu..." : "Pokračovat k platbě"}
                  </Button>
                </form>
